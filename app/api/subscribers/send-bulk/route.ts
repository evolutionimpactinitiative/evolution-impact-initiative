...
export async function POST(request: NextRequest) {
  try {
    ...
    const subscribers = (subscribersData as { id: string; email: string; name: string | null }[] | null) || [];
    ...
    for (let i = 0; i < subscribers.length; i += batchSize) {
      ...
      await Promise.all(
        batch.map(async (subscriber) => {
          try {
            const html = generateBulkEmailHtml(
            ...
            results.failed++;
            results.errors.push(
              `Failed to send to ${subscriber.email}: ${err instanceof Error ? err.message : "Unknown error"}`
            );
          }
        })
      );
    ...
    return NextResponse.json({
      success: true,
      message: `Email sent to ${results.sent} subscriber${results.sent !== 1 ? "s" : ""}`,
      sent: results.sent,
...
function generateBulkEmailHtml(
  ...
  let personalizedBody = body.replace(/Hi there/gi, `Hi ${recipientName}`);
        ...
        <tr>
          <td style="background-color: #ffffff; border-radius: 8px 8px 0 0; padding: 40px; text-align: center;">
            <h1 style="margin: 0 0 25px; font-family: 'Montserrat', sans-serif; font-size: 26px; color: ${BRAND.dark}; font-weight: 900; text-transform: uppercase; letter-spacing: -0.5px;">
              Community<br><span style="color: ${BRAND.blue};">Update</span>
            </h1>
